function rel_to_abs(_location, url){
  ...
  if(/^(https?|file|ftps?|mailto|javascript|data:image\/[^;]{2,9};):/i.test(url))
    ...
    var base_url = _location.href.match(/^(.+)\/?(?:#.+)?$/)[0]+"/";
    if(url.substring(0,2) == "//")
        return _location.protocol + url;
    else if(url.charAt(0) == "/")
        return _location.protocol + "//" + _location.host + url;
...
function replace_all_rel_by_abs(_location, html){
    ...
    function by(match, group1, group2, group3){
        ...
        return group1 + rel_to_abs(_location, group2) + group3;
    }
    /*
      @name by2
